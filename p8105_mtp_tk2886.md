...
### *Research Question*: Does Phone Usage lead to “horns” (EEOP) growing in the back of millenials’ heads?
...
fhp\_category**. However, eop\_shape is not used in the plot creation
process or in my statistical calculation process.
...
There are 303 people in the 18-30 age group with 151 female and 152
male.There are 204 people in the 31-40 age group with 102 female and 102
male. There are 207 people in the 41-50 age group with 106 female and
101 male. There are 200 people in the 51-60 age group with 99 female and
101 male. There are 305 people in the 60+ age group with 155 female and
150 male.
...
eop_dfc %>% 
  ...
  knitr::kable(col.names = c("Gender", "Frequency", "Percentage"),
               caption = "There is a total of 614 female and 607 males participants")
```
...
There is a total of 614 female and 607 males participants
...
eop_dfc %>% 
  ...
  knitr::kable(col.names = c('Age Group', 'Female', 'Male'),
               caption = "Age and Gender Distribution - There is two NA's")
```
...
Age and Gender Distribution - There is two NA’s
...
eop_dfc %>% 
  ...
  summarise(
    frequency = n()) %>%
    mutate(percentage = prop.table(frequency)*100) %>%
  knitr::kable(caption = "No errors or misclassifications")
```
...
No errors or misclassifications
...
eop_dfc %>% 
  ...
  knitr::kable(col.names = c(
    ...
    caption = "EOP Distribution - There is One NA")
```
...
EOP Distribution - There is One NA
...
eop_dfc %>% 
  ...
  knitr::kable(col.names = c(
    ...
    caption = "FHP Distribution - There is One NA")
```
...
FHP Distribution - There is One NA
...
My data analysis findings indicated that there is various errors,
misclassifications, and inconstencies with the data that was provided.
For example, individuals were grouped in the incorrect groups, specific
data values were not correctly labeled, and the calculated statistical
value did not align with the author’s values. The author explained what
each data and figure represents, however the figures and conclusion the
author arrived are not as useful and may not be valid because of the
data misclassification. While my graphs show that younger adults (18-30
years of age) have higher rate of EEOP than other age group, we may
state that there is causal relationship between the usage of cell phone
and horn growth. One data that I would like future researchers to gather
data of how many hours that individual uses cell phone and see if
there’s positive correlation between people who use cell phone
constantly throughout the data compared to people who use cell phones
less frequently.
