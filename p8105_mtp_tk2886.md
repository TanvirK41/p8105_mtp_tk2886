...
``` r
#Improving Figure 3 - Showing the distribution in the boxplot 
eopdf_boxplot <-
  eop_dfc %>%
...
``` r
#Improving Figure 4 - showcasing the rate of EEOP of each of the age group 
rate_plot_df <-
  eop_dfc %>%
  drop_na(age_group) %>%
  group_by(age_group, sex) %>%
