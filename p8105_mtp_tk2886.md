...
eop_dfc %>% 
  ...
  summarise(
    frequency = n()) %>%
    mutate(percentage = prop.table(frequency)*100) %>%
  knitr::kable(caption = "There seems to be **No** errors or misclassifications for EOP Visibility Classification")
```
...
The **reported rate for FHP size over 40mm and among age group of 60+**
is **34.5%**. This is slightly different from the r code calculation:
**32.46%**.
...
My data analysis findings indicated that there is various
inconsistencies with the data that was provided. For example,
individuals were grouped incorrectly, specific data values were not
correctly labeled, and the calculated statistical value did not align
with the author’s values. The figures and conclusion the author arrived
are not as useful and may not be valid because of the data
misclassification. While my graphs show that younger adults (18-30 years
of age) have higher rate of EEOP than other age group, we may not state
that there is causal relationship between the usage of cell phone and
horn growth because of the poor data. I would like researchers to first
gather data on how many hours individual uses cell phone and see if
there’s positive correlation between high usage hours of cell phone and
EEOP.
