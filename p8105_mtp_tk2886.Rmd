...
eop_key_var <- eop_dfc %>%
  names()
```
There are **`r eop_row`** participants in the study. However **2** of the participants are not classified correctly. The key variables in this data set are **`r eop_key_var`**. However, eop_shape is not used in the plot creation process or in my statistical calculation process. 
...
**FHP DISTRIBUTION PLOT:** The average FHP size of male is always larger than female. One interesting trend is that as we analyze older people, the FHP size increases. 


**EEOP RATE PLOT:** The trend of EEOP rate decreased first and then increased for both men and women. EEOP rates is higher in male population than in females among all age groups. The largest rate is in the 18-30 age group and lowest rate is in 41-50 age group. 
...
In each age group for sex there is **slight positive correlation** between FHP size and EOP size. 
...
eop_dfc %>% 
  ...
  knitr::kable(col.names = 
                 c("Age Group",
                   "Participant Count"))
```
The sample size count in four out of the five age group is not consistent with the reported data. **Age group of 51-60** has the *same sample size* as the author's data. 
...
```
The **reported mean of FHP is 26** and this is consistent with the mean that is calculated with the r code which indicates that **26** is the mean. 
...
eop_dfc %>%
  ...
  knitr::kable(
    col.names = c("Sex",
                  ...
                  "Standard Deviation"))
```
The **reported mean of FHP in male is 28** and **for female is 28**. This is consistent with the mean that is calculated with the r code which also indicates that **28** is the mean for men and **24** is the mean for female. 
...
eop_dfc %>%
  ...
  knitr::kable()
```
The **reported mean, standard deviation of each sex in the age group** is *consistent* with the calculated mean and standard deviation from **R**. This was decided by looking at the error bar (Figure 3) in the report. 
...
```
EEOP is enlarged external occipital protuberance and it is *defined* as individuals who have **exceeded 10mm in size for their bony outgrowths**. I used the variable eop_size_mm. I filtered that variable by focusing on individuals who have exceeded 10mm in growth size. The calculated prevalence of **EEOP** in the study population is **32.10%** which is similar to the prevalence reported by the author **33%**. 
...
```
The **reported FHP rate for FHP size over 40mm and among age group of 60+** is  **34.5%** and this is slightly different from the r code calculation: **32.46%**. 
...
```
After calculating the **broad trends**, the data validates the authors statement that FHP is more common in older subjects with FHP size over 40mm and among age group of 60+ because in the *Table FH40*, it is indicated is it more common in the age population of 60+.
...
# **Part 4: Conclusion** 
My data analysis findings indicated that there is various errors, misclassifications, and inconstencies with the data that was provided. For example, individuals were grouped in the incorrect groups, specific data values were not correctly labeled, and the calculated statistical value did not align with the author's values. The author explained what each data and figure represents, however the figures and conclusion the author arrived are not as useful and may not be valid because of the data misclassification. While my graphs show that younger adults (18-30 years of age) have higher rate of EEOP than other age group, we may state that there is causal relationship between the usage of cell phone and horn growth. One data that I would like future researchers to gather data of how many hours that individual uses cell phone and see if there's positive correlation between people who use cell phone constantly throughout the data compared to people who use cell phones less frequently. 
